...
function render(){
  ...
  const done = state.stats.courseDone || cw >= 4;
  setText("pick7", done ? "完成課程" : `課程進度：${cw} / 4`);
...
function isCourseDone(){
  const cw = state.stats?.courseWins ?? 0;
  return !!(state.stats && (state.stats.courseDone || cw >= 4));
}
